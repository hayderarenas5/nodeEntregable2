...
export const makeTransfer=async(req, res)=>{
    try {
        ...
        const [recipientUser,senderUser]= await Promise.all([recipientUserPrimise, senderUserPromise])
        ...
        const updateSenderUserPromise= UserService.updateAmount(recipientUser, newRecipientBalance)
        const updateRecipientUserPromise= UserService.updateAmount(senderUser, newSenderBalance)
        const transferPromise= TransferService.createRecordTransfer(amount, senderUser.id, recipientUser.id)

        await Promise.all([updateSenderUserPromise, updateRecipientUserPromise, transferPromise])
